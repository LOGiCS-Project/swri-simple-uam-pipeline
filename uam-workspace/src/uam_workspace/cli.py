...
def config_example(ctx):
    ...
    log.info(
        ...
        code_2="Config[UAMWorkspaceConfig]",
        result_2=Config[UAMWorkspaceConfig],
    )
    ...
    log.info(
        ...
        The current load paths for PathConfig and UAMWorkspaceConfig:
        """,
        path=[str(p) for p in Config.load_path(PathConfig)],
        uam_workspace=[str(p) for p in Config.load_path(UAMWorkspaceConfig)],
    )
